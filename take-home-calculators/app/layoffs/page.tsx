...
export default function LayoffsPage() {
  ...
  useEffect(() => {
    fetch(SHEET_CSV_URL)
      ...
      .then((csv) => { setRecords(parseCsv(csv)); setLoading(false); })
      .catch((e) => { setError("Could not load data. Please try again shortly."); setLoading(false); console.error(e); });
  }, []);
  ...
  const filtered = useMemo(() => {
    ...
    out = [...out].sort((a, b) => {
      ...
      else if (sortKey === "laid_off") { va = a.laid_off ?? -1; vb = b.laid_off ?? -1; }
      else                         { va = a.percent ?? -1; vb = b.percent ?? -1; }
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
        ...
        <div className="mb-8">
          <div className="flex flex-wrap items-center gap-2 mb-3">
            ...
            {lastUpdated && <span className="text-xs text-ink-soft">Last entry: {lastUpdated}</span>}
          </div>
        ...
        {!loading && !error && (
          <div className="mb-8 grid grid-cols-2 gap-3 sm:grid-cols-4">
            <StatCard label="Total laid off (tracked)" value={totalLaidOff.toLocaleString("en-IN")} sub="across all companies" />
            <StatCard label={`Events in ${thisYear}`} value={thisYearCount.toLocaleString("en-IN")} sub="layoff announcements" />
            <StatCard label="India layoffs" value={indiaLaidOff > 0 ? indiaLaidOff.toLocaleString("en-IN") : "—"} sub="jobs affected" />
            <StatCard label="Companies tracked" value={records.length.toLocaleString("en-IN")} sub="in this dataset" />
          </div>
        )}
            ...
            <div className="overflow-x-auto" style={{ WebkitOverflowScrolling: "touch" } as React.CSSProperties}>
              <table className="w-full text-sm" style={{ minWidth: 680 }}>
                ...
                <tbody>
                  ...
                  {pageRecords.map((r, i) => (
                    <tr key={r.id} className="border-b border-rule last:border-0 transition-colors hover:bg-paper">
                      ...
                      <td className="px-4 py-3 max-w-[180px]">
                        {r.source ? (
                          <a
                            ...
                            title={r.source}
                          >
                            {(() => { try { return new URL(r.source).hostname.replace("www.", ""); } catch { return r.source.slice(0, 30); } })()}
                          </a>
                        ) : "—"}
                      </td>
                    </tr>
          ...
          <div className="mt-4 grid gap-6 sm:grid-cols-2">
            <div>
              <p className="text-ink-soft">
                ...
                affected headcount, date, industry, and a direct link to the source news article.
              </p>
        ...
        <section className="mt-12">
          ...
          <div className="mt-4 space-y-5">
            {[
              { q: "Which Indian IT companies have laid off employees in 2026–27?", a: "Several major IT and tech companies with India operations have announced layoffs in recent months, including companies in Bengaluru, Hyderabad, and Pune. This tracker covers both Indian-headquartered companies and global firms with significant Indian workforces. Filter by country = India to see the full list." },
              { q: "How many tech jobs have been lost in India due to layoffs?", a: `Based on the data tracked here, ${indiaLaidOff > 0 ? indiaLaidOff.toLocaleString("en-IN") + " jobs" : "thousands of roles"} have been affected in India. The actual number is higher as many layoffs go unreported or are announced without specific headcount figures.` },
              { q: "Why are so many tech companies laying off employees?", a: "The wave of tech layoffs since 2022 is primarily driven by: over-hiring during the pandemic-era boom, rising interest rates increasing the cost of capital (especially for VC-backed companies), a shift from growth-at-all-costs to profitability, automation and AI replacing some roles, and post-merger restructuring." },
              { q: "How is this layoff data collected?", a: "Data is sourced from public news reports, company announcements, and verified industry databases. Each entry includes a direct source link so you can verify the information independently." },
              { q: "How often is this tracker updated?", a: "The tracker is updated as new layoff announcements are reported. The data is pulled live from our source sheet, so new entries appear as soon as they are added." },
            ].map((faq) => (
