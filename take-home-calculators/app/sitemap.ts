...
import { salaryGrowthSlug, parseSalaryGrowthSlug } from "@/lib/salary-growth-data";
import { HI_SALARY_GROWTH_LPA_VALUES } from "@/lib/hi-salary-data";
...
export function generateStaticParams() {
  return HI_SALARY_GROWTH_LPA_VALUES.map((lpa) => ({ slug: salaryGrowthSlug(lpa) }));
}
...
export default async function HindiSalaryGrowthPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const nearbyLpas = HI_SALARY_GROWTH_LPA_VALUES.filter(
