// app/calculator/nsc-calculator/page.tsx
"use client";
import { useState, useMemo } from "react";
import Link from "next/link";
import { formatINR } from "@/lib/format";

const NSC_RATE = 7.7; // FY 2025-26 rate
const NSC_TENURE = 5;

function calcNsc(principal: number) {
  const rate = NSC_RATE / 100;
  // NSC compounds annually but interest is deemed reinvested (80C eligible for years 1-4)
  ...
  // Tax saving: principal under 80C + years 1-4 interest reinvested (also 80C)
  return { maturity, totalInterest, breakdown, rate: NSC_RATE };
}
...
export default function NscPage() {
  ...
  // Donut chart SVG
  const total = result.maturity;
  const pct = total ? p / total : 0;
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-10 sm:py-14">
      ...
      <p className="mt-4 text-lg text-ink-soft">Calculate maturity value of NSC at the current {NSC_RATE}% interest rate. NSC has a 5-year lock-in and qualifies for Section 80C deduction.</p>
      ...
      <div className="mt-8 rounded-2xl border border-rule bg-surface p-5 shadow-card">
        ...
        <div className="mt-3 grid grid-cols-3 gap-3">
          {[50000,100000,150000].map(v => (
            <button key={v} onClick={() => setPrincipal(String(v))} className={`rounded-lg border px-3 py-2 text-sm font-medium transition ${p===v?"border-brand bg-brand text-white":"border-rule text-ink-soft hover:border-brand hover:text-brand"}`}>
              ₹{(v/1000).toFixed(0)}K
            </button>
          ))}
        </div>
      ...
      <div className="mt-6 overflow-hidden rounded-2xl border border-rule bg-surface shadow-card-lg">
        ...
        <div className="px-6 py-5 sm:px-8 flex flex-col sm:flex-row items-center gap-6">
          ...
          <div className="flex-1 space-y-2 w-full">
            <div className="flex justify-between border-b border-dashed border-rule py-1.5 text-sm">
              <span className="text-ink-soft">Principal invested</span><span className="tabular font-medium text-ink">{formatINR(p)}</span>
            </div>
            <div className="flex justify-between border-b border-dashed border-rule py-1.5 text-sm">
              <span className="text-ink-soft">Total interest</span><span className="tabular font-medium text-brand">{formatINR(result.totalInterest)}</span>
            </div>
            <div className="flex justify-between border-b border-dashed border-rule py-1.5 text-sm">
              <span className="text-ink-soft">Effective yield</span><span className="tabular font-medium text-ink">{p > 0 ? ((result.totalInterest / p) * 100).toFixed(2) : "0"}%</span>
            </div>
            <div className="flex justify-between py-1.5 text-sm">
              <span className="text-ink-soft">80C eligible (principal)</span><span className="tabular font-medium text-ink">{formatINR(Math.min(p, 150000))}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <section className="mt-10 space-y-4">
        {[
          ...
          {q:"Is NSC interest taxable?",a:"Yes. NSC interest is taxable at your income tax slab rate. However, since interest is deemed reinvested, it qualifies for Section 80C deduction for years 1–4. Only the final year's interest (year 5) is directly taxable without an offsetting 80C benefit."},
          {q:"What is the NSC lock-in period?",a:"NSC has a mandatory 5-year lock-in period. Premature withdrawal is not allowed except in cases of investor death, court order, or forfeiture by a pledgee (such as a bank)."},
        ].map(f=>(
