// app/calculator/step-up-sip-calculator/page.tsx
"use client";
  ...
  return (
    <div className="mt-4">
      <div className="flex items-end gap-1 h-40">
        {data.map(d => (
          <div key={d.year} className="flex-1 flex flex-col items-center gap-0.5 group relative">
            <div className="w-full flex flex-col justify-end" style={{ height: "100%" }}>
              <div className="w-full rounded-t-sm bg-brand-soft group-hover:bg-brand/30 transition-colors"
                style={{ height: `${(d.invested / max) * 100}%` }} />
              <div className="w-full rounded-t-sm bg-brand group-hover:bg-brand-dark transition-colors"
                style={{ height: `${((d.balance - d.invested) / max) * 100}%`, marginBottom: 0 }} />
            </div>
            <span className="text-[9px] text-ink-soft">{d.year}y</span>
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 hidden group-hover:block z-10 bg-ink text-white text-xs rounded px-2 py-1 whitespace-nowrap">
      ...
      <div className="mt-2 flex gap-4 text-xs text-ink-soft">
        <span className="flex items-center gap-1"><span className="inline-block h-2 w-3 rounded-sm bg-brand-soft" />Invested</span>
        <span className="flex items-center gap-1"><span className="inline-block h-2 w-3 rounded-sm bg-brand" />Returns</span>
      </div>
    </div>
  );
}
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-10 sm:py-14">
      <nav className="mb-6 text-sm text-ink-soft" aria-label="Breadcrumb">
        ...
        <Link href="/calculator/sip-calculator" className="hover:text-brand">SIP Calculator</Link><span className="mx-1.5">/</span>
        <span aria-current="page">Step-Up SIP</span>
      </nav>
      <h1 className="font-display text-3xl text-ink sm:text-4xl">Step-Up SIP Calculator</h1>
      <p className="mt-4 text-lg text-ink-soft">Calculate how your SIP grows when you increase your monthly investment by a fixed % every year — the most realistic way to model salary-linked investing.</p>
      ...
      <section className="mt-10 space-y-4">
        {[
          { q: "What is a step-up SIP?", a: "A step-up (or top-up) SIP automatically increases your monthly investment by a fixed percentage every year. For example, starting at ₹10,000/month with a 10% annual step-up means you invest ₹11,000 in year 2, ₹12,100 in year 3, and so on — matching typical salary growth." },
          { q: "Why is step-up SIP better than flat SIP?", a: "Because inflation erodes purchasing power, so a fixed SIP amount actually represents declining real investment over time. A step-up keeps your investment constant in real terms, and the compounding on the incremental amounts significantly boosts the final corpus." },
        ].map(f => (
      ...
      <section className="mt-8"><h2 className="font-display text-xl text-ink">Related</h2>
